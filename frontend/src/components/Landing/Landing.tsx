/**
 * The Kairos front page: what the product is, before the globe.
 *
 * Rendered for a bare URL (no hash). "Launch Kairos" switches to the #app
 * route. Installed-PWA launches skip straight to the app (see App.tsx).
 */
...
const JANUS_PILLARS = [
  {
    ...
    blurb:
      "Reading the remote-sensing literature, framing a research question, and choosing methods that hold up to review.",
  },
  {
    icon: Database,
    name: "Work with real data",
    blurb:
      "Finding and extracting the right datasets, from the Sentinel archives to ground-truth references, and wielding every Kairos tool like an expert.",
  },
  {
    icon: MessagesSquare,
    name: "A mentor that pushes back",
    blurb:
      "Janus questions your reasoning, reviews what you produce, and points you in the right direction, the way a real PhD advisor would.",
  },
];

function JanusWaitlist() {
  ...
  if (status === "done") {
    return (
      <div className="mt-6 flex h-12 max-w-md items-center justify-center rounded-full bg-teal/10 px-6 text-sm text-teal ring-1 ring-teal/40">
        You're on the list. We'll email you when Janus opens.
      </div>
    );
  }
    ...
    <form onSubmit={submit} className="mt-6 max-w-md">
      <div className="flex gap-2">
        <input
          ...
          className="h-12 min-w-0 flex-1 rounded-full bg-bg px-5 text-sm text-ink ring-1 ring-line placeholder:text-dim focus:ring-amber/60 outline-none"
        />
        <button
          type="submit"
          disabled={status === "sending"}
          className="h-12 shrink-0 rounded-full bg-amber px-6 font-medium text-bg transition hover:brightness-110 disabled:opacity-60"
        >
          {status === "sending" ? "Joining…" : "Join the waitlist"}
        </button>
      </div>
      {status === "error" && (
        <p className="mt-2 px-2 text-xs text-[#FF3B5C]">{error}</p>
      )}
    </form>
  );
}
    ...
    <div className="h-full overflow-y-auto overflow-x-hidden bg-bg text-ink">
      ...
      <section className="border-t border-line bg-surface/40">
        <div className="mx-auto max-w-5xl px-6 py-20 sm:px-10">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-relaxed text-dim">
            Kairos answers your questions. Janus teaches you to ask better ones.
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {JANUS_PILLARS.map((p, i) => (
              <motion.div
                ...
                className="rounded-2xl bg-bg p-5 ring-1 ring-line transition-colors hover:ring-amber/40"
              >
          ...
          <p className="mt-3 text-xs text-dim">
            Early testers get Janus free. It will launch as part of a paid plan,
            and the waitlist goes first.
          </p>
        </div>
      </section>
      ...
      <footer className="border-t border-line">
        <div className="mx-auto flex max-w-5xl flex-col gap-2 px-6 py-8 text-xs text-dim sm:px-10">
          <span>Built by the Altis team.</span>
          <span>
